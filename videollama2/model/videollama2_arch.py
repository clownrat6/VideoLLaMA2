...
from ..constants import NUM_FRAMES, IGNORE_INDEX, MMODAL_TOKEN_INDEX
...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos(self, images):
        ...
        data_batch = []
        for i, (modal, data) in enumerate(images):
            if modal == 'image':
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_multimodals = sum((cur_input_ids == mm_token_idx).sum() for mm_token_idx in MMODAL_TOKEN_INDEX.values())
            # pure text input
            ...
            mm_token_indices = torch.where(sum([cur_input_ids == mm_token_idx for mm_token_idx in MMODAL_TOKEN_INDEX.values()]))[0]
