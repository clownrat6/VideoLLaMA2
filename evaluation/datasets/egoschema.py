...
class EgoSchemaDataset(BaseEvalDataset):
    ...
    def load_data(self, data_root: str) -> Dict[int, Any]:
        ...
        for data in data_list:
            ...
            for video_format in ["mp4", "avi", "mov", "mkv"]:
                temp_path = os.path.join(video_folder, f"{question_id}.{video_format}")
    ...
    def evaluate(self, results: List[Dict[str, Any]]) -> (None, Dict[str, List[Dict[str, Any]]]):
        ...
        # file_name = "egoschema-eval-" + "".join(random.choice(characters) for _ in range(16)) + ".csv"
        # save_path = os.path.join(".cache", file_name)
        # os.makedirs(os.path.dirname(save_path), exist_ok=True)
        # with open(save_path, "w") as f:
        #     f.write("q_uid, answer\n")
        #     for result in results:
        #         f.write(f"{result['data_id']}, {result['prediction']}\n")
        ...
        metrics = {
            "Subset": subset_correct / (subset_correct + subset_wrong),
            "Total": total_correct / (total_correct + total_wrong),
        }
